...
class OrderSubmitForm extends OrderForm
{
    ...
    public $use_integral;
    public $formId;
    public $is_water_voucher;
    public $water_deduction_money;
    ...
    public function save()
    {
        ...
        foreach ($mchListData as &$mch) {
            ...
            if($this->is_water_voucher){
                $order->is_water_voucher=1;
                $order->water_deduction=$this->water_deduction_money;
    ...
    //用户水票数量
    private function checkWaterVoucher($goods_id,$num='1'){
        $user_voucher=UserVoucher::find()->where(['goods_id'=>$goods_id,'user_id'=>$this->user_id])->andWhere(['>=','num',$num])->one();
