...
class OrderForm extends ApiModel
{
    ...
    protected function getMchListData($submit = false)
    {
        ...
        foreach ($this->mch_list as $i => &$mch) {
            ...
            if(!empty($mch['goods_list']['0']['type'])&& $mch['goods_list']['0']['type']=='voucher'){
                $this->getGoodsVoucherList($mch['goods_list']);
            }else{
    ...
    //获取商量抵用券
    protected function getGoodsVoucherList(&$goods_list)
    {
        $goodsIds = [];
        foreach ($goods_list as $i => &$item) {
            ...
            $voucher = VoucherPackage::find()
                ->select('*')
                ->where(['goods_id' => $goods->id, 'store_id' => $this->store_id,'id'=>$item['voucher_id']])
                ->asArray()->one();
