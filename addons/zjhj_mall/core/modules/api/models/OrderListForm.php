...
//                    'goods_pic' => $goods->getGoodsPic(0)->pic_url,
            ...
            $orderRefund = OrderRefund::find()->where(['store_id' => $order->store_id, 'order_id' => $order->id])->exists();
             //是否是送水订单
              if($order->is_water_order){
                  $waterorder=$this->waterorder($order->id);
              }
            ...
            $new_list[] = (object)[
                ...
                'refund' => $orderRefund,
                'is_water_order' =>$order->is_water_order,
    ...
    private function waterorder($order_id){
        return WaterOrder::find()->where(['order_id'=>$order_id])->asArray()->one();
    }
