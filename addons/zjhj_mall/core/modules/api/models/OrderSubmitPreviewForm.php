...
//        }
    ...
    private function getDataByGoodsInfo($goods_info, $store)
    {
        ...
        $goods_item = (object)[
            ...
            'integral' => $goods->integral,
            //'weight' => $goods->weight,
			'weight' =>strpos(json_decode(json_encode($attr_list),true)[0]['attr_name'],'g')?floatval(substr(json_decode(json_encode($attr_list),true)[0]['attr_name'],0,-1)):$goods->weight,
            'full_cut' => $goods->full_cut,
            'cat_id' => $goods->cat_id,
            'mch_id' => $goods->mch_id,
        ];
