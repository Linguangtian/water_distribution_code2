...
class OrderSendForm extends MchModel
{
    ...
    {
        return [
            [['express', 'express_no', 'words','water_words'], 'trim'],
            [['express', 'express_no',], 'required', 'on' => 'EXPRESS'],
            [['order_id'], 'required'],
            [['express', 'express_no',], 'string',],
            [['watrman_id','is_express'], 'number',],
    ...
    public function save()
    {
        ...
        if($this->is_express == "2"){
            $order->is_water_order=1;
            $bucket_num=OrderDetail::find()->alias('od')
                        ->leftJoin(['g'=>Goods::tableName()],'od.goods_id=g.id')
                        ->where(['od.order_id'=>$order->id,'g.is_water_bucket'=>1])->select(['sum(od.num) as bucket_num'])->asArray()->one();

            if(empty($bucket_num)||empty($this->watrman_id)){
                return [
                    'code'=>1,
                    'msg'=>'配送员不存在！'
                ];
                ...
                //插入水订单
                $wateroder=new WaterOrder();
                $wateroder->order_id=$order->id;
                $wateroder->waterman_user_id=$this->watrman_id;
                $wateroder->receive_user_id=$order->user_id;
                $wateroder->create_time=date('Y-m-d H:i:s',time());
                $wateroder->status=produce_water_order;
                $wateroder->order_bucket=$bucket_num['bucket_num'];
