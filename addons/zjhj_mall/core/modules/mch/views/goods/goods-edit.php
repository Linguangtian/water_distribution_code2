                ...
                <div>
                    <div class="panel mb-3">
                        ...
                        <div class="panel-body">
                            ...
                            <!--设置抵用券-->
                            <?= $this->render('/layouts/attrs/voucher_setting', [
...
<script>
    $(function () {
        var voucher_data="<?=count($voucher_list)?>";
        if(!voucher_data||voucher_data==0){
            var valueId = getRandomString('v_',9);
            var tr = $(".voucher-row") .find("tr:first-child");
            $(tr).find("input[name='voucher_package[code][]']").val(valueId);
            $(tr).find("input[name='voucher_package[choice][]']").val(valueId);
        }
    })

    $("input[name='setting_voucher']").change(function (e) {
        if($("input[name='setting_voucher']").is(":checked")){
            $(".setting-voucher-list").show();
        }else{
            $(".setting-voucher-list").hide();
        }
    })

    function add_voucher_row() {
        var _div  = $(".voucher-row") .find("tr:last-child").clone();
        $(".voucher-row").append(_div);
        var tr = $(".voucher-row") .find("tr:last-child");
        $(tr).find("input").val('');
        var valueId = getRandomString('v_',9);
        console.info(valueId);
        $(tr).find("input[name='voucher_package[code][]']").val(valueId);
        $(tr).find("input[name='voucher_package[choice][]']").val(valueId);
    ...
    $(document).on("click"," .btn_trash",function(){
        var e = $(this);
        if($(".voucher-row tr").length<=1){
           alert('无法删除请至少保留一行数据');
           return false;
        }
        if(confirm('您确实要删除这行数据吗')){
